...
export async function DELETE(
	...
	try {
		// Llamamos a la función deleteByDni que realiza la baja lógica
		...
		if (success) {
			...
			return NextResponse.json(
				{ error: 'No se pudo realizar la baja lógica del cliente' },
				{ status: 500 }
			)
		}
	} catch (error) {
		...
		return NextResponse.json(
			{ error: 'Hubo un problema al marcar el cliente como eliminado' },
			{ status: 500 }
		)
	}
}
...
export async function PATCH(
	...
	try {
		...
		if (success) {
			...
			return NextResponse.json(
				{ error: 'No se pudo restaurar el cliente' },
				{ status: 500 }
			)
		}
	} catch (error) {
		...
		return NextResponse.json(
			{ error: 'Hubo un problema al restaurar el cliente' },
			{ status: 500 }
		)
	}
}
